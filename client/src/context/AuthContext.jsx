...
export const AuthProvider = ({ children }) => {
  ...
  const hydrate = useCallback(async () => {
    ...
    const selected = available.some((o) => String(o.id) === String(current))
      ? current
      : available[0]?.id || null;
    setOrganizationIdState(selected);
    setStatus("authenticated");
    return { user: me.data, organizations: available, organizationId: selected };
  }, []);
  ...
  const setOrganizationId = useCallback((value) => {
    setOrganizationIdState(value || null);
  ...
  const workspaceRole =
    membership?.role || (user?.role === "admin" ? "platform_admin" : user?.role);
