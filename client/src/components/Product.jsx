...
export const PageHeader = ({ eyebrow, title, description, action }) => (
  <header className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div>
      ...
      <h1 className="mt-1 text-2xl font-bold tracking-tight text-ink-950 sm:text-3xl">{title}</h1>
      {description && (
        <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-500 sm:text-base">{description}</p>
      )}
    </div>
    {action && <div className="shrink-0">{action}</div>}
  </header>
);
export const Metric = ({ label, value, detail, tone = "ink" }) => (
...
export const AIProvenance = ({ metadata, confidence, limitations = [] }) => (
  <div className="rounded-xl border border-white/10 bg-white/5 p-4 text-sm">
    <div className="flex flex-wrap items-center gap-2">
      <Bot className="h-4 w-4 text-cyan-300" />
      <span className="font-semibold">AI result</span>
      {confidence !== undefined && confidence !== null && (
        <Badge variant="outline">{Math.round(confidence * 100)}% confidence</Badge>
      )}
      {metadata?.fallbackUsed && <Badge variant="warning">Deterministic fallback</Badge>}
    </div>
    {metadata && (
      <p className="mt-2 text-xs text-ink-400">
        {metadata.provider} · {metadata.model} · prompt {metadata.promptVersion}
      </p>
    )}
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
        <div
          className={cn(
            "grid h-28 w-28 shrink-0 place-items-center rounded-full border-8 border-white shadow-sm",
            scoreTone(match.overallScore),
          )}
        >
          <div className="text-center">
            <div className="text-3xl font-bold">{match.overallScore}</div>
            <div className="text-[10px] font-bold uppercase">fit score</div>
          </div>
        </div>
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-lg font-bold capitalize">
              {String(match.recommendation || "Fit analysis").replaceAll("_", " ")}
            </h3>
            <Badge variant="outline">{Math.round((match.confidence || 0) * 100)}% confidence</Badge>
          </div>
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {Object.entries(components).map(([key, value]) => (
              <div key={key} className="rounded-xl border border-ink-200 bg-white p-3">
                <div className="flex justify-between text-xs">
                  <span className="font-semibold capitalize text-ink-600">
                    {key.replace(/([A-Z])/g, " $1")}
                  </span>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1 text-sm text-ink-600">
                ...
                {!match.concerns?.length && <li>No major concerns identified.</li>}
              </ul>
            </div>
          </div>
          <div className="rounded-xl bg-ink-50 p-4 text-sm text-ink-600">
            <p className="font-semibold text-ink-900">Evidence & limitations</p>
            <p className="mt-1">
              Semantic method: {match.semanticEvidence?.method || "not available"}. Experience:{" "}
              {match.experienceEvidence?.candidateYears ?? "uncertain"} years against{" "}
              {match.experienceEvidence?.requiredYears ?? "no explicit minimum"}.
            </p>
            <ul className="mt-2">
...
export const StatusPill = ({ status }) => {
  ...
  const [Icon, variant] = map[s] || [Info, "default"];
  return (
...
export const JobTile = ({ job, match, saved, onSave }) => {
  ...
  return (
    <article className="panel p-5 transition hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        {match && (
          <div className={cn("rounded-xl px-3 py-2 text-center", scoreTone(match.overallScore))}>
            <div className="text-xl font-bold">{match.overallScore}</div>
            <div className="text-[10px] font-bold uppercase">fit</div>
          </div>
        )}
      </div>
      ...
      <div className="mt-5 flex items-center gap-2">
        <Button as={Link} to={href} size="sm">
          View fit <ArrowRight className="h-3.5 w-3.5" />
        </Button>
        {onSave && (
