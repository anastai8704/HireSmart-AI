...
const PublicJobCard = ({ job }) => {
  ...
  const chips = [
    ...
    job.jobType,
    job.experience && !job.minExpYears ? job.experience : null,
    job.minExpYears
      ? `${job.minExpYears}${job.maxExpYears ? `–${job.maxExpYears}` : "+"} yrs`
      : null,
  ].filter(Boolean);
  return (
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-500">
            {job.organization?.slug ? (
              <Link
                to={`/companies/${job.organization.slug}`}
        ...
        {salary && (
          <span className="shrink-0 text-sm font-bold text-ink-900">
            {salary}
            <span className="block text-right text-[10px] font-medium text-ink-400">per year</span>
          </span>
        )}
      </div>
...
export const PublicJobsPage = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (debouncedKeyword.trim() !== currentQuery) setParam("query", debouncedKeyword.trim());
    }, 60);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedKeyword]);
  const requestParams = Object.fromEntries(
    FILTER_KEYS.filter((k) => get(k)).map((k) => [k, get(k)]),
  );
  const q = useInfiniteQuery({
    queryKey: ["jobs-public", paramsKey],
    queryFn: ({ pageParam, signal }) =>
      jobsApi.list(
        { limit: 12, ...requestParams, ...(pageParam ? { after: pageParam } : {}) },
        { signal },
      ),
  ...
  const createAlert = async (event) => {
    ...
    try {
      const body = {
        name: alertName.trim() || "My job alert",
        cadence: alertCadence,
        query: get("query"),
        location: get("location"),
        workplaceMode: get("workplaceMode"),
        jobType: get("jobType"),
      ...
      <main className="page-wrap">
        <div className="rounded-3xl bg-ink-950 px-5 py-8 text-white sm:px-10">
          ...
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              if (keyword.trim() !== get("query")) setParam("query", keyword.trim());
              if (location.trim() !== get("location")) setParam("location", location.trim());
            }}
          >
            <Input
            ...
            <div className="mx-auto mt-3">
              <form onSubmit={runNlSearch} className="flex gap-2">
                <Input
                  ...
                  placeholder='Ask AI: "remote react job 15 lpa in pune"'
                  icon={<Sparkles className="h-4 w-4" />}
                  value={nlText}
                  onChange={(e) => setNlText(e.target.value)}
                  className="!border-white/10 !bg-white/6 !text-white"
                />
                <Button type="submit" variant="secondary" isLoading={nlPending}>
                  Ask AI
                </Button>
              </form>
            ...
            <div className="mt-4 space-y-5 text-sm">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      key={label}
                      type="button"
                      onClick={() => setParam("workplaceMode", value)}
                      className={`rounded-full px-3 py-1 text-xs font-medium transition ${get("workplaceMode") === value ? "bg-ink-950 text-white" : "bg-ink-50 text-ink-600 hover:bg-ink-100"}`}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={get("maxSalary")}
                    onChange={(e) => setParam("maxSalary", e.target.value)}
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  value={get("skills")}
                  onChange={(e) => setParam("skills", e.target.value)}
                />
              </div>
              <div>
          ...
          <section>
            <div className="flex items-end justify-between">
              <div>
                ...
                <h2 className="mt-1 text-xl font-bold">
                  {items.length || "…"} opportunity{items.length === 1 ? "" : "ies"}
                </h2>
              </div>
              <div className="flex items-center gap-2">
                {isCandidate && (
                  <Button size="sm" variant="ghost" onClick={() => setShowAlertForm((v) => !v)}>
                ...
                <Select
                  ...
                  value={get("sort") || "date"}
                  onChange={(e) =>
                    setParam("sort", e.target.value === "date" ? "" : e.target.value)
                  }
                  options={[
            ...
            {isCandidate && showAlertForm && (
              <form
                ...
                className="panel mt-3 flex flex-wrap items-end gap-3 p-4"
              >
                <div className="min-w-48 flex-1">
                  <Input
                    label="Alert name"
                    placeholder="e.g. React roles in Pune"
                    value={alertName}
                ...
                <div>
                  <Select
                    label="Frequency"
                    value={alertCadence}
                    onChange={(e) => setAlertCadence(e.target.value)}
                    options={[
                ...
                <Button type="submit" isLoading={alertPending}>
                  Create alert
                </Button>
              </form>
            )}
            <div className="mt-4 grid gap-4">
              {q.isLoading && items.length === 0 ? (
                ...
                items.map((job) => <PublicJobCard key={job.id} job={job} />)
              ) : !q.isLoading ? (
                <EmptyState
                  title="No matching jobs"
                  description="Try removing a filter or broadening your location."
                />
              ) : null}
            </div>
            {q.hasNextPage && (
...
export const PublicJobDetailPage = () => {
  ...
  const jsonLd = job
    ? {
        ...
        validThrough:
          job.closesAt ||
          new Date(new Date(job.createdAt).getTime() + 60 * 24 * 3600 * 1000).toISOString(),
          ...
          job.compensation && (job.compensation.min || job.compensation.max)
            ? {
                ...
                value: {
                  ...
                  max: job.compensation.max || job.compensation.min || 0,
                  unitText:
                    job.compensation.period === "month"
                      ? "MONTH"
                      : job.compensation.period === "hour"
                        ? "HOUR"
                        : "YEAR",
                },
              }
    ...
    : null;
  usePageMeta({
    title: job ? `${job.title} at ${job.company} — HireSmart AI` : "Job — HireSmart AI",
    description: job
      ? `${job.title} in ${job.location}. ${job.description?.slice(0, 150)}…`
      : undefined,
    jsonLd,
  });
            ...
            <article className="panel p-7">
              <div className="flex items-start justify-between gap-4">
                ...
                {isCandidate && (
                  <button
                    ...
                    onClick={toggleSave}
                    disabled={saving}
                    className="rounded-xl border border-ink-200 px-3 py-2 text-sm font-semibold text-ink-600 transition hover:border-brand-300 hover:text-brand-700"
                  >
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {salary && (
                  <div className="rounded-xl bg-ink-50 p-4">
                    <p className="text-xs font-semibold uppercase text-ink-400">Salary / year</p>
                    <p className="mt-1 font-bold">{salary}</p>
                  </div>
                )}
                <div className="rounded-xl bg-ink-50 p-4">
                  ...
                  <p className="mt-1 font-bold">
                    {job.minExpYears
                      ? `${job.minExpYears}${job.maxExpYears ? `–${job.maxExpYears}` : "+"} years`
                      : job.experience || "Any"}
                  </p>
                </div>
                <div className="rounded-xl bg-ink-50 p-4">
                  <p className="text-xs font-semibold uppercase text-ink-400">Type</p>
                  <p className="mt-1 font-bold">
                    {job.jobType}
                    {job.workplaceMode && job.workplaceMode !== "unspecified"
                      ? ` · ${job.workplaceMode}`
                      : ""}
                  </p>
                </div>
              ...
              {job.educationRequired && (
                <p className="mt-4 flex items-center gap-2 text-sm text-ink-600">
                  <GraduationCap className="h-4 w-4 text-ink-400" />
                  Education: {job.educationRequired}
                </p>
              )}
              <h2 className="mt-7 text-lg font-bold">About the role</h2>
              <div className="mt-3 whitespace-pre-wrap leading-7 text-ink-700">
                {job.description}
              </div>
                ...
                <>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    ...
                    {(job.preferredSkills || []).map((skill) => (
                      <span
                        key={skill}
                        className="rounded-md bg-ink-50 px-2.5 py-1 text-xs font-medium text-ink-500"
                      >
                        {skill} (nice to have)
                      </span>
                    ))}
                ...
                <>
                  ...
                  <ul className="mt-3 space-y-2">
                    {job.benefits.map((benefit) => (
                      <li key={benefit} className="flex items-center gap-2 text-sm text-ink-600">
            ...
            <aside className="space-y-4">
              <div className="panel p-5">
                {isCandidate ? (
                  ...
                  <>
                    ...
                    <p className="mt-2 text-sm text-ink-500">
                      You're signed in as {auth.role}. Applying needs a candidate account.
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <Button
                      as={Link}
                      to={`/auth/login?next=/jobs/${job.id}`}
                      fullWidth
                      className="mt-4"
                    >
                      Sign in to apply
              ...
              <div className="panel p-5 text-sm">
                ...
                <dl className="mt-3 space-y-2 text-ink-600">
                  ...
                  <div className="flex justify-between">
                    <dt className="text-ink-400">Work mode</dt>
                    <dd className="font-medium capitalize">
                      {job.workplaceMode || "Not specified"}
                    </dd>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <dd className="font-medium">
                      {job.closesAt ? formatRelativeTime(job.closesAt) : "Rolling"}
                    </dd>
                  </div>
                </dl>
              </div>
...
export const CompaniesPage = () => {
  ...
  usePageMeta({
    title: "Companies hiring — HireSmart AI",
    description: "Browse companies hiring on HireSmart and see their open roles.",
  });
  return (
    <>
      <Navbar />
      <main className="page-wrap">
        ...
        {q.isLoading ? (
          ...
          <>
            <p className="mt-2 text-sm text-ink-500">
              Companies with the most open roles, updated live.
            </p>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {q.data.data.map((c) => (
                <Link
                  key={c.id}
                  to={`/companies/${c.slug}`}
                  className="panel p-5 transition hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-sm"
                >
                  <div className="flex items-center gap-4">
                    {c.logo ? (
                      <img src={c.logo} alt="" className="h-12 w-12 rounded-xl object-contain" />
                    ) : (
                      <span className="grid h-12 w-12 place-items-center rounded-xl bg-ink-950 text-base font-bold text-white">
                        {(c.name || "?").slice(0, 2).toUpperCase()}
                      </span>
                    )}
                    <div>
                      ...
                      <p className="text-xs text-ink-500">
                        {[c.industry, c.size && `${c.size} people`].filter(Boolean).join(" · ") ||
                          "Company"}
                      </p>
                    </div>
                  </div>
            ...
            {!q.data.data.length && (
              <div className="mt-8">
                <EmptyState
                  ...
                  description="Employers appear here once they publish jobs."
                />
              </div>
            )}
          </>
...
export const CompanyPage = () => {
  ...
  const jobs = useQuery({
    ...
    queryFn: () => companiesApi.jobs(slug, { limit: 12 }),
  });
  const c = company.data?.data;
  usePageMeta({
    title: c ? `${c.name} — open roles on HireSmart AI` : "Company — HireSmart AI",
    description: c ? `${c.about?.slice(0, 150)} or see ${c.openRoles} open roles.` : undefined,
  });
  return (
    <>
      <Navbar />
      <main className="page-wrap">
        {company.isLoading ? (
          ...
          <ErrorState error={company.error} />
        ) : (
          <>
            <header className="panel flex flex-col gap-5 p-7 sm:flex-row sm:items-center">
              {c.logo ? (
                <img src={c.logo} alt="" className="h-20 w-20 rounded-2xl object-contain" />
              ) : (
                <span className="grid h-20 w-20 place-items-center rounded-2xl bg-ink-950 text-2xl font-bold text-white">
                  {(c.name || "?").slice(0, 2).toUpperCase()}
                </span>
              )}
              <div className="flex-1">
                ...
                {c.about && (
                  <p className="mt-3 max-w-3xl text-sm leading-6 text-ink-600">{c.about}</p>
                )}
              </div>
            ...
            <section className="mt-8">
              ...
              <div className="mt-4 grid gap-4">
                {jobs.isLoading ? (
                  ...
                  jobs.data.data.map((job) => <PublicJobCard key={job.id} job={job} />)
                ) : (
                  <EmptyState
                    title="No open roles right now"
                    description="Check back soon — or set an alert from the jobs page."
                  />
                )}
              </div>
            </section>
