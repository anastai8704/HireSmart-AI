...
test("parseSrvRecords ignores malformed entries", () => {
    assert.deepEqual(parseSrvRecords(["", "0 0", "no-port b.mongodb.net", "0 0 27017 ok.mongodb.net"]), [
        { target: "ok.mongodb.net", port: 27017 },
    ]);
});

test("buildDirectSrvUri joins the shards and appends TLS by default", () => {
    const parsed = parseSrvUri("mongodb+srv://u:s@cluster.klvtcut.mongodb.net/hiresmart?retryWrites=false");
    const records = [
        { target: "a.mongodb.net", port: 27017 },
        { target: "b.mongodb.net", port: 27017 },
    ];
...
test("isSrvDnsFailure triggers only on SRV-URI DNS layer failures", () => {
    ...
    assert.equal(isSrvDnsFailure(srvUri, new Error("querySrv ECONNREFUSED _mongodb._tcp.cluster.klvtcut.mongodb.net")), true);
    ...
    assert.equal(isSrvDnsFailure(srvUri, Object.assign(new Error("getaddrinfo EAI_AGAIN cluster.mongodb.net"), { errno: "EAI_AGAIN" })), true);
    assert.equal(isSrvDnsFailure(srvUri, new Error("connect ECONNREFUSED 127.0.0.1:27017")), false);
    assert.equal(isSrvDnsFailure(plainUri, new Error("querySrv ECONNREFUSED _mongodb._tcp.cluster.klvtcut.mongodb.net")), false);
