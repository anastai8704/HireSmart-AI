...
test("marketplace: org owner publishes structured jobs for the public", async () => {
  ...
  jobIds.react = (
    await createPublishedJob({
      ...
      location: "Pune",
      compensation: { min: 1500000, max: 2400000, currency: "INR", period: "year" },
      experience: "4+ years",
      minExpYears: 4,
      maxExpYears: 8,
      educationRequired: "B.Tech / B.E.",
      benefits: ["Health insurance", "Remote Fridays"],
      jobType: "Full-Time",
      workplaceMode: "remote",
      description:
        "Build delightful, accessible product surfaces with React, TypeScript and modern testing.",
  ...
  jobIds.node = (
    await createPublishedJob({
      ...
      company: "Meridian Labs",
      location: "Pune",
      compensation: { min: 900000, max: 1500000, currency: "INR", period: "year" },
      experience: "2+ years",
      minExpYears: 2,
      jobType: "Full-Time",
      workplaceMode: "hybrid",
      description:
        "Design resilient Node.js services and MongoDB data models powering a recruitment platform.",
  ...
  jobIds.devops = (
    await createPublishedJob({
      ...
      company: "Meridian Labs",
      location: "Bengaluru",
      compensation: { min: 2000000, max: 3200000, currency: "INR", period: "year" },
      experience: "5+ years",
      minExpYears: 5,
      maxExpYears: 10,
      jobType: "Contract",
      ...
      description:
        "Own CI/CD, cloud infrastructure and observability for production Kubernetes workloads.",
  ...
  const draft = await request(app)
    ...
    .send({
      ...
      workplaceMode: "onsite",
      description:
        "An unpublished role that must never appear on public search results or company pages.",
...
test("marketplace: public search filters by salary, experience, type, skills and work mode", async () => {
  ...
  res = await request(app).get(
    `/api/v1/jobs?${new URLSearchParams({ workplaceMode: "remote", jobType: "Contract", minSalary: 1800000 }).toString()}`,
  );
  assert.ok(!res.body.data.map((j) => j.id).includes(jobIds.react));
...
test("marketplace: organization updates are whitelisted (mass-assignment protection)", async () => {
  const res = await request(app)
    ...
    .send({
      ...
      about: "We build hiring infrastructure.",
    });
  assert.equal(res.status, 200, JSON.stringify(res.body));
  ...
  assert.equal(
    ...
    "active",
    "status must not be assignable through the update endpoint",
  );
  ...
  const withSettings = await request(app)
    .patch(`/api/v1/organizations/${organizationId}`)
    .set(auth(ownerToken))
    .send({ name: "Meridian Labs Pvt Ltd", settings: { aiEnabled: false } });
...
test("marketplace: tenant isolation — another org cannot see these jobs in company listing", async () => {
  ...
  const created = await request(app)
    .post(`/api/v1/organizations/${other.organization.id}/jobs`)
    .set(auth(other.token))
    .send({
      title: "Private Other Role",
      company: "Other Co",
      location: "Delhi",
      experience: "1 year",
      jobType: "Full-Time",
      workplaceMode: "onsite",
      description:
        "A job from a different organization that must stay isolated in every public surface.",
      requiredSkills: ["Java"],
    });
