...
let candidateToken; let candidateUserId; let ownerToken; let organizationId;
...
test("phase 4: seed candidate with ready resume and two published jobs", async () => {
    await clearDatabase();
    const owner = await registerAndLogin({ email: "owner@snapshots.example", intent: "recruiter", organizationName: "Snapshot Co" });
    ownerToken = owner.token; organizationId = owner.organization.id;
    const candidate = await registerAndLogin({ email: "candidate@snapshots.example", intent: "candidate" });
    candidateToken = candidate.token; candidateUserId = candidate.user.id;
    for (const payload of [
        { title: "React Engineer", company: "Snapshot Co", location: "Pune", experience: "3+ years", jobType: "Full-Time", workplaceMode: "remote", description: "Build and maintain accessible React frontends with TypeScript and automated testing coverage.", requiredSkills: ["React", "TypeScript"] },
        { title: "Node.js Engineer", company: "Snapshot Co", location: "Pune", experience: "2+ years", jobType: "Full-Time", workplaceMode: "hybrid", description: "Design resilient Node.js services with MongoDB and comprehensive integration test suites.", requiredSkills: ["Node.js", "MongoDB"] },
    ]) {
...
test("phase 4: first call is live, then snapshots are precomputed and served from cache", async () => {
    const first = await request(app).get("/api/v1/candidates/me/recommendations").set(auth(candidateToken));
    ...
    const second = await request(app).get("/api/v1/candidates/me/recommendations").set(auth(candidateToken));
...
test("phase 4: prompt injection returns only validated structured output", async () => {
    ...
    assert.ok(!answer.toLowerCase().includes("untrusted user-provided"), "system prompt content must not leak");
    assert.ok(!answer.toLowerCase().includes("hiresmart's recruitment assistant"), "system prompt content must not leak (2)");
