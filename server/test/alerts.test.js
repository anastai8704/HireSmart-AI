...
test("phase 2: seed candidate, owner and two published jobs", async () => {
  ...
  const owner = await registerAndLogin({
    email: "owner@alerts.example",
    intent: "recruiter",
    organizationName: "Alert Co",
  });
  ownerToken = owner.token;
  organizationId = owner.organization.id;
  const candidate = await registerAndLogin({
    email: "candidate@alerts.example",
    intent: "candidate",
  });
...
test("phase 2: creating an alert and running the scan delivers matching jobs once", async () => {
  const created = await request(app)
    ...
    .send({ name: "React roles", skills: ["React"], workplaceMode: "remote", cadence: "daily" });
  ...
  await JobAlert.updateOne(
    { _id: alertId },
    { $set: { lastRunAt: new Date(Date.now() - 25 * 3600 * 1000) } },
  );
  ...
  const deactivated = await request(app)
    .patch(`/api/v1/candidates/me/alerts/${alertId}`)
    .set(auth(candidateToken))
    .send({ active: false });
  assert.equal(deactivated.status, 200);
  await request(app)
    .post(`/api/v1/organizations/${organizationId}/jobs/${reactJobId}/close`)
    .set(auth(ownerToken));
  const republished = await request(app)
    ...
    .send({
      ...
      company: "Alert Co",
      location: "Bengaluru",
      compensation: { min: 2000000, max: 3000000 },
      experience: "6+ years",
      jobType: "Full-Time",
      workplaceMode: "remote",
      description:
        "Lead the React platform team across multiple product surfaces and mentoring loops.",
      requiredSkills: ["React", "TypeScript"],
    });
...
test("phase 2: non-matching alert delivers nothing", async () => {
  const created = await request(app)
    .post("/api/v1/candidates/me/alerts")
    .set(auth(candidateToken))
    .send({ name: "Data jobs", skills: ["Data Science"], location: "Chennai" });
...
test("phase 2: authenticated public search is recorded in search history", async () => {
  ...
  const list = await request(app)
    .get("/api/v1/candidates/me/search-history")
    .set(auth(candidateToken));
  ...
  const cleared = await request(app)
    .delete("/api/v1/candidates/me/search-history")
    .set(auth(candidateToken));
...
test("phase 2: natural-language search produces structured filters (deterministic)", async () => {
  const response = await request(app)
    ...
    .set(auth(candidateToken))
    .send({ input: { text: "remote react job 15 lpa in pune" } });
...
test("phase 2: recommendations are personalized with saved/search signals", async () => {
  ...
  const saved = await request(app)
    .post(`/api/v1/candidates/me/saved-jobs/${nodeJobId}`)
    .set(auth(candidateToken));
  ...
  assert.ok(
    recommendations.body.meta.signals.savedSkillCount >= 1,
    "saved-job skills should feed the signal",
  );
  assert.ok(
    recommendations.body.data.every((item) => String(item.job.id) !== String(nodeJobId)),
    "saved job must not be recommended",
  );
});
