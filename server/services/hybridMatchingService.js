...
const semanticScore = async (resumeText, jobText, allowExternalEmbeddings = false) => {
    ...
    if (allowExternalEmbeddings && config.embeddingsProvider !== "deterministic") {
        try {
            const provider = getProvider(config.embeddingsProvider); const [a, b] = await Promise.all([provider.embed(resumeText), provider.embed(jobText)]);
...
const calculateHybridMatch = async ({ resumeText = "", candidateSkills = [], job, candidatePreferences = {}, allowExternalEmbeddings = false }) => {
    ...
    const w = config.matchingWeights; const overallScore = Math.round(Object.entries(components).reduce((sum, [k, v]) => sum + v * w[k], 0) / Object.values(w).reduce((a, b) => a + b, 0));
