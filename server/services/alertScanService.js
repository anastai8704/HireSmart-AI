...
const runAlert = async (alert) => {
    ...
    const since = alert.lastRunAt ? new Date(alert.lastRunAt.getTime() - 3600 * 1000) : new Date(Date.now() - 30 * 24 * 3600 * 1000);
    const jobs = await Job.find({ ...filter, createdAt: { $gte: since } }).sort({ createdAt: -1 }).limit(200).select("_id title company location");
    ...
    for (const job of fresh) {
        try {
            await notify({
                user: alert.user, type: "job_alert",
                title: `New job match: ${job.title}`,
                message: `${job.title} at ${job.company} (${job.location}) matches your alert “${alert.name}”.`,
                resourceType: "job", resourceId: job._id, email: user?.email || null,
                idempotencyKey: `alert:${alert._id}:${job._id}`,
            });
        } catch (error) {
            logger.error(`Job alert ${alert._id} -> job ${job._id} notification failed: ${error.code || error.message}`);
        }
    }
    await JobAlert.updateOne({ _id: alert._id }, { $set: { deliveredJobIds: trimDelivered([...(alert.deliveredJobIds || []), ...fresh.map((job) => String(job._id))]) } });
    return fresh.length;
};
...
const runAlertScan = async () => {
    ...
    if (scanned) logger.info(`Alert scan complete: ${scanned} alerts scanned, ${delivered} job matches delivered`);
