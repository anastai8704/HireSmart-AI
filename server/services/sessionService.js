...
const createSession = async (user, req) => {
    ...
    const session = await AuthSession.create({ user: user._id, tokenHash: hash(token), csrfHash: hash(csrfToken), familyId: crypto.randomUUID(), userAgent: req.get("user-agent") || "", ipHash: hashIp(req.ip), expiresAt: new Date(Date.now() + config.refreshTokenExpiresDays * 86400000) });
...
const rotateSession = async (rawToken, req) => {
    ...
    if (!session || session.revokedAt || session.expiresAt <= new Date()) throw new AppError("Refresh session is invalid", 401, "SESSION_INVALID");
    ...
    await session.save();
    return { session, user, refreshToken: next, csrfToken: nextCsrf, accessToken: accessToken(user, session) };
};
const revoke = async (query, reason) => AuthSession.updateMany({ ...query, revokedAt: null }, { revokedAt: new Date(), revokeReason: reason });
const cookieOptions = () => ({ httpOnly: true, secure: config.cookieSecure, sameSite: config.cookieSameSite, path: "/api/v1/auth", maxAge: config.refreshTokenExpiresDays * 86400000 });
