 ...
 * -----------------------------------------------------------------------------
 * Grades a resume the way a real Applicant Tracking System (ATS) would, and
 * tells the candidate exactly what to fix.
 *
 * WHY THIS MODULE MATTERS
 * Over 90% of large employers filter resumes through an ATS before a human ever
 * reads them. Candidates get rejected for mechanical reasons - no contact
 * details, no measurable achievements, missing keywords, a two-column layout the
 * parser could not read. This module runs those same checks and returns
 * actionable advice instead of a silent rejection.
 *
 * It is the "Resume Analyzer" half of the product name, and it works with no
 * job attached (general health check) or against a specific job (targeted gap
 * analysis).
 */
...
const SKILL_DICTIONARY = Object.freeze({
    languages: [
        "javascript", "typescript", "python", "java", "csharp", "cplusplus", "c",
        "go", "rust", "ruby", "php", "swift", "kotlin", "scala", "r", "dart",
    ],
    frontend: [
        "react", "angular", "vue", "next", "svelte", "html", "css", "tailwind",
        "bootstrap", "sass", "redux", "jquery", "webpack", "vite",
    ],
    backend: [
        "node", "express", "django", "flask", "fastapi", "spring", "laravel",
        "rails", "dotnet", "graphql", "restapi", "microservices", "grpc",
    ],
    database: [
        "mongodb", "postgresql", "mysql", "sqlite", "redis", "elasticsearch",
        "oracle", "cassandra", "dynamodb", "firebase", "sql", "nosql",
    ],
    devops: [
        "docker", "kubernetes", "amazonwebservices", "azure", "googlecloud",
        "jenkins", "cicd", "terraform", "ansible", "nginx", "linux", "git",
        "github", "gitlab",
    ],
    datascience: [
        "machinelearning", "deeplearning", "artificialintelligence", "tensorflow",
        "pytorch", "pandas", "numpy", "scikit", "keras", "opencv", "nlp",
        "tableau", "powerbi", "spark", "hadoop",
    ],
    testing: [
...
const ACTION_VERBS = [
    ...
    "launched", "delivered", "architected", "migrated", "scaled", "shipped",
    "engineered", "managed", "mentored", "collaborated", "resolved",
    "streamlined", "integrated", "deployed", "refactored", "achieved",
];

/** Phrases that signal vague, low-impact writing. */
const WEAK_PHRASES = [
    "responsible for", "worked on", "helped with", "involved in",
    "duties included", "familiar with", "exposure to", "team player",
    "hard worker", "go-getter", "think outside the box", "detail oriented",
];
...
const extractContactInfo = (text) => {
    ...
    const phone = text.match(/(?:\+\d{1,3}[\s-]?)?(?:\(\d{2,4}\)[\s-]?)?\d{3,5}[\s-]?\d{3,5}(?:[\s-]?\d{2,4})?/);
...
const findQuantifiedAchievements = (text) => {
    ...
    const quantified = lines.filter((line) =>
        /\d+\s*%|\d+\s*x\b|\$\s*\d|₹\s*\d|\b\d{2,}\s*(?:users|customers|requests|records|hours|days|ms|seconds|k\b|million|lakh|crore)/i.test(line)
    );

    return { totalLines: lines.length, quantified: quantified.slice(0, 10), count: quantified.length };
};
...
const runChecks = (text) => {
    ...
    return {
        ...
        checks: [
            ...
            { key: "achievements", label: "Quantified achievements", score: clamp(achievementScore), weight: 0.2 },
...
const buildSuggestions = (analysis) => {
    ...
    if (!contact.email) {
        suggestions.push({
            ...
            detail: "No email was found. An ATS cannot forward your application without one. Put it in the top three lines of the resume.",
        });
    }
    ...
    if (!contact.phone) {
        suggestions.push({
            ...
            detail: "Recruiters shortlist by phone first. Include your number with the country code, for example +91 98765 43210.",
        });
    }
    ...
    if (!contact.linkedin) {
        suggestions.push({
            ...
            detail: "Around 87% of recruiters check LinkedIn. Add the full URL so it stays clickable after PDF export.",
        });
    }
    ...
    if (!contact.github && !contact.portfolio) {
        suggestions.push({
            ...
            detail: "For a developer role, working code is the strongest proof of skill. Link a GitHub profile with pinned projects.",
        });
    }
    ...
    if (actionVerbs.length < 5) {
        suggestions.push({
            ...
            detail: "Open each bullet with verbs such as Built, Designed, Automated, Reduced or Led instead of passive descriptions.",
        });
    }
...
const analyzeResume = (resumeText = "") => {
    ...
    if (text.trim().length < 50) {
        return {
            ...
            grade: "F",
            wordCount: 0,
            summary: "We could not read enough text from this resume. If it is a scanned image or uses an unusual layout, export a text-based PDF and upload it again.",
    ...
    const strongestCategory =
        Object.entries(analysis.skills.byCategory).sort(
