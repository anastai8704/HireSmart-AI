...
const buildRecommendations = async (userId) => {
    ...
    const version = await ResumeVersion.findOne({ candidate: userId, processingStatus: "ready" }).sort({ createdAt: -1 }).select("+text");
    if (!version) throw new AppError("Upload and process a resume first", 422, "RESUME_REQUIRED");
    const allowExternalEmbeddings = Boolean(await Consent.exists({ user: userId, purpose: "ai_processing", revokedAt: null }));
    const parsed = await ParsedResume.findOne({ resumeVersion: version._id }).select("skills");
    const versionSkills = parsed?.skills?.map((item) => item.normalized || item.name) || user.skills || [];
    ...
    const signals = { savedSkillCount: savedSkillSet.length, searchTermCount: terms.length, appliedExcluded: appliedIds.length };
    return { resumeVersionId: version._id, items: scored.slice(0, 20), signals, evaluated: scored.length };
};

const refreshSnapshot = async (userId) => {
    const built = await buildRecommendations(userId);
    await RecommendationSnapshot.findOneAndUpdate({ user: userId }, { $set: { resumeVersionId: built.resumeVersionId, results: built.items, signals: built.signals, evaluated: built.evaluated, computedAt: new Date() } }, { upsert: true });
...
const tickRecommendationRefresh = async (force = false) => {
    ...
    try {
        ...
        const users = await User.find({ _id: { $in: candidates }, role: "candidate", isActive: true, updatedAt: { $gte: new Date(now - 60 * 24 * 3600 * 1000) } }).select("_id").limit(300).lean();
        const pending = await JobRun.find({ type: "recommendations.refresh", status: { $in: ["queued", "processing"] } }).select("owner");
        const pendingOwners = new Set(pending.map((j) => String(j.owner)));
        let enqueued = 0;
        for (const u of users) {
            if (pendingOwners.has(String(u._id))) continue;
            await enqueue({ type: "recommendations.refresh", owner: u._id, payload: { userId: String(u._id) }, maxAttempts: 2 });
            pendingOwners.add(String(u._id));
            enqueued += 1;
        }
        if (enqueued) logger.info(`Recommendation refresh tick: ${enqueued} snapshots enqueued (${users.length} eligible)`);
