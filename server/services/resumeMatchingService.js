// Scores how well a resume fits a job, as four explainable components:
// skills 55%, semantic (TF-IDF cosine) 25%, experience 15%, education 5%.
// Pure and synchronous - no database, no network.
...
const scoreSkills = ({ jobSkills, resumeSkills, resumeTokens }) => {
  ...
  for (const skill of requiredSkills) {
    ...
    const isInResumeText =
      skillTokens.length > 0 && skillTokens.every((token) => resumeTokenSet.has(token));
...
const scoreExperience = ({ jobExperience, resumeText, candidateYears }) => {
  ...
  const actual =
    typeof candidateYears === "number" ? candidateYears : extractYearsOfExperience(resumeText);
...
const buildExplanation = ({ skills, semantic, experience, education }) => {
  ...
  if (skills.applicable) {
    ...
    reasons.push(`Matched ${skills.matchedSkills.length} of ${total} required skills.`);
  ...
  if (semantic.applicable) {
    const quality = semantic.score >= 70 ? "closely" : semantic.score >= 40 ? "partly" : "loosely";
