...
const membershipSchema = new mongoose.Schema({
    ...
    status: { type: String, enum: ["invited", "active", "suspended", "revoked"], default: "active", index: true },
...
membershipSchema.methods.hasPermission = function hasPermission(permission) {
    return this.status === "active" && (permissionsByRole[this.role] || []).concat(this.permissions).includes(permission);
};
