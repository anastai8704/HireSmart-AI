...
resumeSchema.index({ candidate: 1, isPrimary: 1 }, { unique: true, partialFilterExpression: { isPrimary: true, status: "active" } });
...
const resumeVersionSchema = new mongoose.Schema({
    ...
    processingStatus: { type: String, enum: ["queued", "processing", "ready", "partial", "failed", "rejected", "deleted"], default: "queued", index: true },
    processingStage: { type: String, enum: ["uploaded", "validated", "parsing", "extracting", "analyzing", "complete"], default: "uploaded" },
...
const parsedResumeSchema = new mongoose.Schema({
    resumeVersion: { type: mongoose.Schema.Types.ObjectId, ref: "ResumeVersion", required: true, unique: true, index: true },
    candidate: { type: mongoose.Schema.Types.ObjectId, ref: "User", required: true, index: true },
    schemaVersion: { type: String, default: "1.0" },
    contact: { email: String, phone: String, linkedin: String, github: String, portfolio: String },
    skills: [{ name: { type: String, required: true }, normalized: { type: String, required: true }, confidence: { type: Number, min: 0, max: 1, default: 0.7 }, evidence: String }],
    ...
    experiences: [{ title: String, company: String, startDate: Date, endDate: Date, description: String, evidence: String }],
