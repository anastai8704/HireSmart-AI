...
const firstMeaningfulMessage = (error) =>
    [error?.message, error?.reason?.message, error?.cause?.message]
        .find((value) => typeof value === "string" && value.length > 0) || String(error || "unknown error");
...
const isSrvDnsFailure = (uri, error) => {
    ...
    return /querySrv|queryA|getaddrinfo|EAI_AGAIN|ENOTFOUND/i.test(message) || error.errno === "EAI_AGAIN";
};
...
const connectWithRetryPolicy = async (uri) => {
    try {
        ...
        logger.warn("MongoDB SRV lookup is still failing. Falling back to DNS-over-HTTPS resolution...");
...
const connectDB = async (uriOverride) => {
    ...
    try {
        ...
        logger.info(
            mode === "doh" ? "MongoDB connected successfully (DNS-over-HTTPS fallback)" : "MongoDB connected successfully"
        );
    } catch (error) {
