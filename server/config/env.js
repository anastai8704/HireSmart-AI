...
const config = Object.freeze({
  ...
  processJobsInline:
    String(process.env.PROCESS_JOBS_INLINE).toLowerCase() === "true" || nodeEnv === "test",
  ...
  matchingWeights: {
    ...
    preferredSkills: numberEnv("MATCH_WEIGHT_PREFERRED_SKILLS", 0.1),
    experience: numberEnv("MATCH_WEIGHT_EXPERIENCE", 0.2),
    education: numberEnv("MATCH_WEIGHT_EDUCATION", 0.1),
    semantic: numberEnv("MATCH_WEIGHT_SEMANTIC", 0.2),
...
const validateEnvironment = () => {
  ...
  if (config.isProduction) {
    ...
    if (!config.s3Bucket || !config.s3Region)
      throw new Error("S3_BUCKET and S3_REGION are required in production");
    if (!config.malwareScannerUrl) throw new Error("MALWARE_SCANNER_URL is required in production");
    if (!config.smtpHost || !config.smtpUser || !config.smtpPass)
      throw new Error(
        "SMTP_HOST, SMTP_USER and SMTP_PASS are required when production email verification is enabled",
      );
    if (!["strict", "lax", "none"].includes(String(config.cookieSameSite).toLowerCase()))
    ...
    if (config.aiPrimaryProvider !== "deterministic" && !config.aiApiKey)
      throw new Error("AI_API_KEY is required for the configured primary AI provider");
    if (
      config.aiFallbackProvider !== "deterministic" &&
      !config.aiFallbackApiKey &&
      !config.aiApiKey
    )
      throw new Error(
        "AI_FALLBACK_API_KEY or AI_API_KEY is required for the configured fallback provider",
      );
    if (
      config.embeddingsProvider !== "deterministic" &&
      !config.embeddingsApiKey &&
      !config.aiApiKey
    )
      throw new Error(
        "EMBEDDINGS_API_KEY or AI_API_KEY is required for the configured embeddings provider",
      );
  }
  ...
  if (
    ...
    throw new Error("Matching weights must be finite non-negative numbers with a positive total");
