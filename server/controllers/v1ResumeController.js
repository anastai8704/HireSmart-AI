...
exports.remove = asyncHandler(async (req, res) => {
    ...
    if (resume && String(resume.currentVersion) === String(version._id)) {
        const replacement = await ResumeVersion.findOne({ resume: resume._id, processingStatus: "ready", _id: { $ne: version._id } }).sort({ version: -1 }).select("+storageKey +text");
        resume.currentVersion = replacement?._id || null; await resume.save();
        if (resume.isPrimary) {
            const User = require("../models/User");
            await User.updateOne({ _id: req.user._id }, replacement ? { resume: replacement.storageKey, resumeProvider: replacement.storageProvider, resumeOriginalName: replacement.originalName, resumeMimeType: replacement.mimeType, resumeSize: replacement.size, resumeUploadedAt: replacement.createdAt, resumeText: replacement.text, resumeSummary: replacement.summary } : { resume: "", resumeOriginalName: "", resumeMimeType: "", resumeSize: 0, resumeUploadedAt: null, resumeText: "", resumeSummary: "" });
        }
    }
