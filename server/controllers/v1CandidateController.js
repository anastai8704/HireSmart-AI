...
const hasAIConsent = (userId) =>
  Consent.exists({ user: userId, purpose: "ai_processing", revokedAt: null });
...
exports.update = asyncHandler(async (req, res) => {
  ...
  for (const key of userFields) if (req.body[key] !== undefined) req.user[key] = req.body[key];
  await req.user.save();
  const profileFields = [
    "city",
    "state",
  ...
  const profile = Object.keys(profileUpdate).length
    ...
    : await CandidateProfile.findOne({ user: req.user._id });
  res.json({
    data: {
      user: {
        id: req.user._id,
...
exports.fit = asyncHandler(async (req, res) => {
  ...
  const result = await calculateHybridMatch({
    ...
    job,
    candidatePreferences: req.body.preferences || {},
    allowExternalEmbeddings: Boolean(await hasAIConsent(req.user._id)),
  });
  res.json({ data: { jobId: job._id, resumeVersionId: version._id, ...result } });
});
...
exports.saveJob = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ data: { jobId: job._id, saved: true } });
});
exports.unsaveJob = asyncHandler(async (req, res) => {
  await User.updateOne({ _id: req.user._id }, { $pull: { savedJobs: req.params.jobId } });
  res.status(204).end();
});
exports.recommendations = asyncHandler(async (req, res) => {
  ...
  const excludedIds = new Set([
    ...appliedJobs.map((a) => String(a.job)),
    ...(userDoc?.savedJobs || []).map(String),
  ]);
  const excludeEngaged = (items) =>
    (items || []).filter((item) => !excludedIds.has(String(item?.job?.id)));
  const snapshot = await RecommendationSnapshot.findOne({ user: req.user._id });
  if (
    snapshot &&
    String(snapshot.resumeVersionId) === String(version._id) &&
    Date.now() - snapshot.computedAt.getTime() < 24 * 3600 * 1000
  ) {
    return res.json({
      data: excludeEngaged(snapshot.results),
      meta: {
        source: "snapshot",
  ...
  res.json({
    data: built.items.slice(0, Math.min(Number(req.query.limit) || 20, 50)),
    meta: {
      source: "live",
      evaluated: built.evaluated,
      resumeVersionId: version._id,
