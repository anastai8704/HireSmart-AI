...
exports.execute = asyncHandler(async (req, res) => {
  if (!features.has(req.params.feature))
    throw new AppError("AI feature not found", 404, "RESOURCE_NOT_FOUND");
  if (recruiterOnly.has(req.params.feature) && !["recruiter", "admin"].includes(req.user.role))
  ...
  if (req.params.feature === "career_copilot")
    input = { ...input, authorizedContext: await candidateContext(req.user._id) };
  if (req.params.feature === "recruiter_copilot")
    input = { ...input, authorizedContext: await recruiterContext(req.auth.organizationId) };
