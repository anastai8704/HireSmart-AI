...
exports.settings = asyncHandler(async (req, res) => { const org = await Organization.findOne({ _id: req.auth.organizationId }); if (!org) throw new AppError("Resource not found", 404, "RESOURCE_NOT_FOUND"); if (typeof req.body.requireJobApproval === "boolean") org.settings.requireJobApproval = req.body.requireJobApproval; await org.save(); await audit({ req, organization: org._id, action: "organization.settings.updated", resourceType: "organization", resourceId: org._id, metadata: { requireJobApproval: org.settings.requireJobApproval } }); res.json({ data: { id: org._id, name: org.name, slug: org.slug, settings: org.settings } }); });
