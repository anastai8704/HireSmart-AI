...
const loadPendingInvite = async (token) => {
    const invitation = await Invite.findOne({ token }).populate("invitedBy", "name").populate("organization", "name");
    if (!invitation || invitation.status !== "pending") throw new AppError("This invitation is no longer valid", 404, "INVITE_INVALID");
    if (invitation.expiresAt <= new Date()) throw new AppError("This invitation has expired. Ask the person who invited you to send a new one.", 410, "INVITE_EXPIRED");
    return invitation;
};
...
exports.accept = asyncHandler(async (req, res) => {
    ...
    if (await User.exists({ email })) {
        throw new AppError("An account with this email already exists. Sign in and accept the invitation instead.", 409, "EMAIL_IN_USE");
    }
    ...
    await audit({ req, organization: invitation.organization._id, action: "invitation.accepted", resourceType: "invitation", resourceId: invitation._id, metadata: { role: invitation.role } });
    res.status(201).json({
        data: { organization: { id: invitation.organization._id, name: invitation.organization.name }, role: membership.role },
    });
});
...
exports.acceptExisting = asyncHandler(async (req, res) => {
    ...
    if (String(req.user.email).toLowerCase() !== invitation.email) {
        throw new AppError("This invitation was sent to a different email address", 403, "INVITE_EMAIL_MISMATCH");
    }
    const membership = await Membership.findOneAndUpdate(
        { organization: invitation.organization._id, user: req.user._id },
        { role: invitation.role, status: "active", invitedBy: invitation.invitedBy, joinedAt: new Date() },
