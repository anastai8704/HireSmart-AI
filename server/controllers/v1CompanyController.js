...
exports.companies = asyncHandler(async (req, res) => {
    const grouped = await Job.aggregate([
        { $match: { status: "published", organization: { $ne: null }, $or: [{ closesAt: null }, { closesAt: { $gte: new Date() } }] } },
...
exports.company = asyncHandler(async (req, res) => {
    ...
    const openRoles = await Job.countDocuments(openRolesFilter(org._id));
...
exports.companyJobs = asyncHandler(async (req, res) => {
    ...
    const filter = applyCursor(openRolesFilter(org._id), page.after);
