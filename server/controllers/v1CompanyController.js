...
const openRolesFilter = (organization) => ({
  organization,
  status: "published",
  $or: [{ closesAt: null }, { closesAt: { $gte: new Date() } }],
});
const companyDto = (org, openRoles) => ({
  id: org._id,
  name: org.name,
  slug: org.slug,
...
exports.companies = asyncHandler(async (req, res) => {
  ...
  const data = grouped
    ...
    .map((g) => companyDto(byId.get(String(g._id)), g.count));
  res.json({ data, meta: { count: data.length } });
});

exports.company = asyncHandler(async (req, res) => {
  const org = await Organization.findOne({
    slug: String(req.params.slug).slice(0, 100),
    status: "active",
  });
...
exports.companyJobs = asyncHandler(async (req, res) => {
  ...
  const filter = applyCursor(
    { ...openRolesFilter(org._id), ...(await moderationGate()) },
    page.after,
  );
