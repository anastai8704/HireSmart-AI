...
exports.list = asyncHandler(async (req, res) => {
  const items = await JobAlert.find({ user: req.user._id }).sort({ createdAt: -1 }).limit(50);
  res.json({ data: items.map(alertDto), meta: { count: items.length } });
});
...
exports.create = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ data: alertDto(alert) });
});
...
exports.remove = asyncHandler(async (req, res) => {
  const deleted = await JobAlert.findOneAndDelete({ _id: req.params.alertId, user: req.user._id });
  if (!deleted) throw new AppError("Alert not found", 404, "RESOURCE_NOT_FOUND");
  res.status(204).send();
});
