...
const run = async () => {
    ...
    while (!stopping) { const worked = await processNext(); if (!worked) { await wait(1000); tickAlertScan().catch((error) => logger.error(`Alert scan tick failed: ${error.message}`)); tickRecommendationRefresh().catch((error) => logger.error(`Recommendation tick failed: ${error.message}`)); } }
    await disconnectDB();
};
