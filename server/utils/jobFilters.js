...
const buildPublicJobFilter = (query = {}) => {
    ...
    if (minSalary) and.push({ $or: [{ "compensation.max": { $gte: minSalary } }, { "compensation.max": { $lt: 1 }, salary: { $gte: minSalary } }] });
    if (maxSalary) and.push({ $or: [{ "compensation.min": { $lte: maxSalary } }, { "compensation.min": { $lt: 1 }, salary: { $lte: maxSalary } }] });
    ...
    const skillList = String(query.skills || query.skill || "").split(",").map((s) => s.trim()).filter(Boolean).slice(0, 10);
    if (skillList.length) and.push({ $or: skillList.map((s) => ({ $or: [{ requiredSkills: s }, { preferredSkills: s }, { skills: s }] })) });
    const posted = String(query.postedWithin || "").toLowerCase();
    if (["d", "w", "m"].includes(posted)) { const hours = posted === "d" ? 24 : posted === "w" ? 24 * 7 : 24 * 30; filter.createdAt = { $gte: new Date(Date.now() - hours * 3600 * 1000) }; }
    if (and.length) filter.$and = and;
    if (query.query) filter.$text = { $search: String(query.query).slice(0, 100) };
    const sortKey = String(query.sort || "date").toLowerCase();
    const sort = sortKey === "salary" ? { "compensation.min": -1, _id: -1 } : sortKey === "relevance" && query.query ? { score: "DESC", _id: -1 } : { _id: -1 };
    return { filter, sort };
};
