...
const CANDIDATES = [
    {
        ...
        education: [{ institution: "Nirma University", degree: "B.E.", fieldOfStudy: "Computer Engineering", startYear: 2015, endYear: 2019, cgpa: 8.4 }],
        jobs: [
            {
                company: "Zycus",
        ...
        projects: [{ title: "OpenCart UI Kit", description: "Open-source React component library with 2.1k GitHub stars, full accessibility and theming support.", technologies: ["React", "TypeScript", "Storybook"] }],
    ...
    },
    {
        ...
        skills: ["Node.js", "PostgreSQL", "MongoDB", "Redis", "AWS", "REST APIs", "Docker", "Microservices"],
        ...
        educationLine: "B.Tech Information Technology, Visvesvaraya Technological University (2016 - 2020)",
        education: [{ institution: "Visvesvaraya Technological University", degree: "B.Tech", fieldOfStudy: "Information Technology", startYear: 2016, endYear: 2020, cgpa: 8.1 }],
        jobs: [
            {
        ...
        projects: [{ title: "ledger-sync", description: "Open-source idempotent ledger reconciliation worker for Node.js (800 GitHub stars).", technologies: ["Node.js", "PostgreSQL", "Redis"] }],
    ...
    },
    {
        ...
        education: [{ institution: "COEP Pune", degree: "B.E.", fieldOfStudy: "Computer Science", startYear: 2015, endYear: 2019, cgpa: 7.9 }],
        jobs: [
            {
        ...
        projects: [{ title: "cluster-doctor", description: "CLI that audits EKS cluster health (node pressure, storage, cert expiry) and posts a Grafana snapshot.", technologies: ["Go", "Kubernetes", "Grafana"] }],
        certifications: [{ name: "CKA: Certified Kubernetes Administrator", issuer: "CNCF" }, { name: "HashiCorp Certified: Terraform Associate", issuer: "HashiCorp" }],
    },
    {
        name: "Arjun Mehta",
        ...
        educationLine: "B.E. Computer Engineering, L.D. College of Engineering, Ahmedabad (2020 - 2024)",
        education: [{ institution: "L.D. College of Engineering", degree: "B.E.", fieldOfStudy: "Computer Engineering", startYear: 2020, endYear: 2024, cgpa: 8.8 }],
        jobs: [
            {
        ...
        projects: [{ title: "shopfront-kit", description: "Collection of accessible e-commerce React components (cart, filters, product grid) with docs site.", technologies: ["React", "Tailwind", "Jest"] }],
        certifications: [],
    },
    {
        name: "Ishita Verma",
        ...
        education: [{ institution: "Symbiosis Institute of Technology", degree: "B.Tech", fieldOfStudy: "Computer Science", startYear: 2017, endYear: 2021, cgpa: 8.0 }],
        jobs: [
            {
        ...
        projects: [{ title: "flaky-triage", description: "Playwright reporter plugin that groups flaky tests and links them to CI logs.", technologies: ["Playwright", "TypeScript"] }],
    ...
    },
    {
        ...
        education: [{ institution: "GUJCET University", degree: "B.E.", fieldOfStudy: "Computer Engineering", startYear: 2015, endYear: 2019, cgpa: 7.7 }],
        jobs: [
            {
        ...
        projects: [{ title: "billpay-cli", description: "Internal CLI that queries billing state across services with a single command.", technologies: ["Node.js", "TypeScript"] }],
    ...
    },
    {
        ...
        education: [{ institution: "Mumbai University", degree: "B.E.", fieldOfStudy: "Computer Science", startYear: 2016, endYear: 2020, cgpa: 7.5 }],
        jobs: [
            {
        ...
        projects: [{ title: "slo-report", description: "Weekly SLO report generator pulling Prometheus metrics into Slack digests.", technologies: ["Python", "Prometheus", "Slack API"] }],
...
const APPLICATIONS = [
    {
        ...
        notes: [["kavya", "Hands-on with GraphQL and testing. Salary expectation 30 LPA - within band.", ["compensation"]]],
    },
    {
    ...
    },
    {
        ...
        history: [
            ...
            ["under_review", "kavya", "QA background but strong JavaScript fundamentals; reviewing for fit.", 3],
        ],
        notes: [],
    },
    {
        ...
        notes: [["kavya", "Prepare system-design questions around idempotency and queues.", ["interview-prep"]]],
    },
    {
        ...
        history: [
            ...
            ["under_review", "kavya", "CKA + Terraform Associate with real multi-cluster experience.", 22],
    ...
    },
    {
        ...
        history: [
            ...
            ["under_review", "kavya", "Good SRE instincts, but needs deeper Terraform experience for current scope.", 13],
            ["rejected", "kavya", "Not a fit this cycle; added to talent pool for future IAC-heavy roles.", 10],
        ],
        notes: [],
    },
];
...
const resumeLinesFor = (c) => [
    ...
    "",
    "EXPERIENCE",
    ...c.jobs.map((j) => `${j.position}, ${j.company} (${j.from.slice(0, 4)} - ${j.to ? j.to.slice(0, 4) : "present"}): ${j.note}`),
    "",
    "EDUCATION",
    c.educationLine,
];
...
const cleanupLegacy = async () => {
    ...
    const apps = await Application.find({ $or: [{ candidate: { $in: userIds } }, { organization: { $in: orgIds } }] });
    const appIds = apps.map((a) => a._id);
    const jobs = await Job.find({ $or: [{ recruiter: { $in: userIds } }, { organization: { $in: orgIds } }] });
    ...
    await run("CandidateMatch", () => CandidateMatch.deleteMany({ $or: [{ organization: { $in: orgIds } }, { application: { $in: appIds } }, { job: { $in: jobIds } }] }));
    await run("Note", () => Note.deleteMany({ $or: [{ organization: { $in: orgIds } }, { author: { $in: userIds } }, { targetType: "application", targetId: { $in: appIds } }] }));
    await run("Interview", () => Interview.deleteMany({ $or: [{ organization: { $in: orgIds } }, { application: { $in: appIds } }] }));
    await run("JobRun", () => JobRun.deleteMany({ organization: { $in: orgIds } }));
    await run("AIAnalysis", () => AIAnalysis.deleteMany({ $or: [{ organization: { $in: orgIds } }, { user: { $in: userIds } }] }));
    await run("Notification", () => Notification.deleteMany({ $or: [{ organization: { $in: orgIds } }, { user: { $in: userIds } }] }));
    await run("SecurityEvent", () => SecurityEvent.deleteMany({ $or: [{ organization: { $in: orgIds } }, { user: { $in: userIds } }] }));
    await run("AuditLog", () => AuditLog.deleteMany({ $or: [{ organization: { $in: orgIds } }, { actor: { $in: userIds } }] }));
    ...
    await run("Membership", () => Membership.deleteMany({ $or: [{ user: { $in: userIds } }, { organization: { $in: orgIds } }] }));
    ...
    console.log(`  Removed documents: ${Object.entries(counts).filter(([, n]) => n > 0).map(([k, n]) => `${k}=${n}`).join(", ") || "none"}`);
    console.log(`  Resume files     : ${files} deleted from disk`);
    if (orgs.some((o) => o.slug === "demo-tech-solutions")) {
        console.log("  NOTE: 'Demo Tech Solutions' was removed - any manual members you added there (e.g. your own account) lost that membership. Your user accounts are untouched.");
    }
...
const createWorld = async () => {
    ...
    for (const t of TEAM) {
        ...
        if (!m) {
            m = await Membership.create({ organization: org._id, user: u._id, role: t.membershipRole, status: "active" });
    ...
    for (const c of CANDIDATES) {
        ...
        for (const purpose of ["terms", "privacy", "ai_processing"]) {
            if (!(await Consent.exists({ user: u._id, purpose, policyVersion: "2026-08" }))) {
                await Consent.create({ user: u._id, purpose, policyVersion: "2026-08", source: "registration" });
            }
        }
    ...
    for (const c of CANDIDATES) {
        ...
        let version = await ResumeVersion.findOne({ candidate: u._id, processingStatus: "ready" }).select("+storageKey");
        if (!version) {
    ...
    for (const a of APPLICATIONS) {
        ...
        for (const n of a.notes) {
            const exists = await Note.findOne({ organization: org._id, targetType: "application", targetId: app._id, body: n[1] });
            if (!exists) {
                await Note.create({
                    organization: org._id,
                    targetType: "application",
                    targetId: app._id,
                    author: teamUsers[n[0] === "kavya" ? "Kavya Mehta" : n[0] === "rohit" ? "Rohit Deshmukh" : "Aarav Gupta"]._id,
                    body: n[1],
            ...
            feedback: [
                {
                    ...
                    summary: "Strong production Kubernetes experience; walked through a real incident calmly and thoroughly. Would definitely like to work with her.",
                    ratings: [
                        { criterion: "Kubernetes", score: 5, evidence: "Runs multi-cluster EKS with GitOps rollouts and HPA tuning" },
                        { criterion: "Terraform", score: 4, evidence: "Solid module design; limited exposure to remote state backends" },
                        { criterion: "AWS", score: 4, evidence: "Deep EC2/RDS/CloudFront experience, comfortable with IAM design" },
                        { criterion: "Communication", score: 4, evidence: "Structured answers, concise incident narration" },
                    ],
                },
                {
                    evaluator: "Rohit Deshmukh",
                    recommendation: "yes",
                    summary: "Great fit for the platform team. Recommend offer at 24 LPA with a joining date in 3 weeks.",
                    ratings: [
                        { criterion: "Terraform", score: 4, evidence: "Real module libraries with CI validation" },
                        { criterion: "CI/CD", score: 4, evidence: "GitLab + ArgoCD progressive delivery" },
                        { criterion: "Troubleshooting", score: 5, evidence: "Led a 45-minute outage diagnosis end-to-end" },
                        { criterion: "Communication", score: 4, evidence: "Clear, no fluff; asks good clarifying questions" },
                    ],
                },
            ],
        },
...
const main = async () => {
    ...
    try {
        ...
        console.log("----------------------------------------------------------");
        for (const j of JOBS) {
            console.log(`  [${j.status.padEnd(9)}] ${j.title} - ${j.location} - INR ${Math.round(j.compensation.min / 100000)}-${Math.round(j.compensation.max / 100000)} LPA`);
        }
        console.log("");
