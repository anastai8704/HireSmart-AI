...
const main = async () => {
  ...
  try {
    ...
    let resumeLine;
    const existingReady = await ResumeVersion.findOne({
    ...
    console.log(
      `Company        : ${org.name} (slug: ${org.slug})${orgNew ? "  [created]" : "  [reused]"}`,
    );
    console.log(
      `Recruiter login: ${DEMO.recruiter.email}  /  ${DEMO.recruiter.password}${recruiterNew ? "  [created]" : "  [reused]"}`,
    );
    console.log(
      `Job posted     : ${job.title} - ${job.location}${jobNew ? "  [created]" : "  [reused]"}`,
    );
    console.log(
      `Candidate login: ${DEMO.candidate.email}  /  ${DEMO.candidate.password}${candidateNew ? "  [created]" : "  [reused]"}`,
    );
    ...
    console.log(
      "1. Log in as candidate -> Resumes (ready) -> Discover -> open the job -> Analyze fit -> Apply",
    );
    console.log(
      "2. Log in as recruiter -> the applicant appears -> shortlist / add note / schedule interview",
    );
